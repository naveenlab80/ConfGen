...
def convert_excel_to_junos(excel_file, output_file=None):
    ...
    try:
        ...
        elif serial_number and hostname:
            final_output_file = f"{hostname}_{serial_number}_config.txt"
        elif serial_number:
            final_output_file = f"config_{serial_number}.txt"
        elif hostname:
            final_output_file = f"{hostname}_config.txt"
        else:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("      based on hostname and serial number: <hostname>_<serial>_config.txt")
        sys.exit(1)
