...
- **Auto-naming**: Generated config files are automatically named as `<hostname>_<serial>_config.txt`
...
**Basic Usage** (auto-generates filename based on hostname and serial number):

```bash
python juniper_excel_to_config.py juniper_config_template.xlsx
```

Output: `ex-switch-01_ABC123456789_config.txt`
...
- **Hostname + Serial**: `<hostname>_<serial>_config.txt` (e.g., `ex-switch-01_ABC123456789_config.txt`)
- **Serial only**: `config_<serial>.txt` (if hostname is missing)
- **Hostname only**: `<hostname>_config.txt` (if serial is missing)
